...
minimal_vivado_srcs_path = str(Path(minimal_char_bitstream_path).parent / 'Vivado_Sources')

# full characterization paths
full_TCs = Path(minimal_char_bitstream_path).glob('*.bit')
full_vivado_srcs_path = str(Path(minimal_char_bitstream_path).parent / 'Vivado_Sources')

for iteration in range(cycles):
    minimal_char_result = Path(store_path).parent / 'Results' / 'minimal' / f'iter{iteration}'
    full_char_result = Path(store_path).parent / 'Results' / 'minimal' / f'iter{iteration}'

    # create folder
    minimal_char_result.mkdir(parents=True)
    full_char_result.mkdir(parents=True)
    ...
    for TC in minimal_TCs:
        os.system(
            f'{cfg.python} {char_experiment_script} {N_Parallel} {COM_port} {baud_rate} {minimal_char_bitstream_path} {TC.stem} {str(full_char_result)} {full_vivado_srcs_path}')
